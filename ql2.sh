...
if [ "$access" != "2" ]; then
    if [ "$(grep -c "token" $CONFIG_PATH/auth.json)" != 0 ]; then
        log "7.开始安装或重装 Ninja"
        if [ "$INSTALL_NINJA" = true ]; then
            docker exec -it $CONTAINER_NAME bash -c "cd /ql/data;ps -ef|grep ninja|grep -v grep|awk '{print $1}'|xargs kill -9;rm -rf /ql/data/ninja;git clone https://git.metauniverse-cn.com/https://github.com/shufflewzc/Waikiki_ninja.git /ql/data/ninja;cd /ql/data/ninja/backend;pnpm install;cp .env.example .env;cp sendNotify.js /ql/data/scripts/sendNotify.js;sed -i \"s/ALLOW_NUM=40/ALLOW_NUM=100/\" /ql/data/ninja/backend/.env;pm2 start"
            docker exec -it $CONTAINER_NAME bash -c "sed -i \"s/ALLOW_NUM=40/ALLOW_NUM=100/\" /ql/data/ninja/backend/.env && cd /ql/data/ninja/backend && pm2 start"
        fi
        log "8.开始青龙内部配置"
        docker exec -it $CONTAINER_NAME bash -c "$(curl -fsSL https://gt.ererttw.top/https://raw.githubusercontent.com/xjy0320/ceshi/main/custom.sh)"
    else
