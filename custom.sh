...
# 获取有效 config.sh 链接
get_valid_config() {
    config_list=(https://gt.ererttw.top/https://raw.githubusercontent.com/xjy0320/fakerVIP/main/Conf/Qinglong/config.sample.sh)
...
# 获取有效 extra.sh 链接
get_valid_extra() {
    extra_list=(https://gt.ererttw.top/https://raw.githubusercontent.com/xjy0320/fakerVIP/main/Tasks/qlrepo/extra.sh)
...
# 获取有效 code.sh 链接
get_valid_code() {
    code_list=(https://gt.ererttw.top/https://raw.githubusercontent.com/xjy0320/fakerVIP/main/Scripts/sh/Helpcode2.8/code.sh)
...
add_task_code() {
    if [ "$(grep -c "code.sh" /ql/data/config/crontab.list)" != 0 ]; then
        ...
        # 获取token
        token=$(cat /ql/data/config/auth.json | jq --raw-output .token)
        curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"格式化更新助力码","command":"task /ql/data/config/code.sh","schedule":"55 * * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1626247939659'
    fi
}
if [ "${all}" = 1 ]; then
...
get_valid_task_before() {
    task_before_list=(https://gt.ererttw.top/https://raw.githubusercontent.com/xjy0320/fakerVIP/main/Scripts/sh/Helpcode2.8/task_before.sh)
